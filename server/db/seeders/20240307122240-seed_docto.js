...
module.exports = {
  async up(queryInterface, Sequelize) {
    const entries = new Array(30).fill(null).map(() => ({
      ...
      car_id: randomIntFromInterval(1, 6), // Предполагая, что у вас есть машины с ID от 1 до 5
      dateNow: randomDate(60, 0), // Случайная дата от сегодня до 60 дней назад
      probegKm: randomIntFromInterval(0, 100000), // Случайный пробег
      status: randomBoolean(), // Случайный статус
      userComment: questions[randomIntFromInterval(0, questions.length - 1)],
      ourComment: randomManagerComment(),
      userScore: randomUserScore(), // Случайный userScore с 30% шансом на null
      createdAt: new Date(),
      updatedAt: new Date(),
    }));
